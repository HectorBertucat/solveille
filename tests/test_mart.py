"""Test du mart commune_pression : LEFT joins, E (gating pas-d'argile), J, flag bascule.
...
(gating) ; pas de prix DVF → valeur NULL ; non listée dans bascule → basculement FALSE.
"""
...
from solveille.metric.ip_rga import exposition_e
from solveille.transform.mart import build_commune_pression
...
def mart(tmp_path: Path) -> Path:
    ...
    build_commune_pression(staging_dir=stg, out=out)
    return out
...
def test_commune_exposed_with_value(mart: Path) -> None:
    ...
    assert a["ip_rga_score"] is None  # pas de T en v0
